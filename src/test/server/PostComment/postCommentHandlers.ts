...
export const postCommentHandlers = [
  ...
  http.post<any, { post_id: number, message: string }>(FULL_URL, async ({ request }) => {
    ...
    inMemoryResponse.data = [newPostCommentAPI, ...inMemoryResponse.data],
      inMemoryResponse.meta = { ...inMemoryResponse.meta, total: inMemoryResponse.meta.total + 1 }
  ...
  http.delete<{postCommentId: string}>(`${FULL_URL}/:postCommentId`, async ({ params }) => {
    ...
    inMemoryResponse.data = inMemoryResponse.data.filter(item => item.id.toString() !== postCommentId)
